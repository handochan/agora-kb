...
| Native Windows | **Not supported — cannot run** | `src/agora_kb/curator/claim.py:30` imports `fcntl` unconditionally, on the path of every command. The `agora` entry point now refuses with three lines and exit 2 rather than a traceback ([#103](https://github.com/handochan/agora-kb/issues/103), `src/agora_kb/_entry.py`) — a message, not a port. Port is epic [#85](https://github.com/handochan/agora-kb/issues/85); `windows-latest` is `continue-on-error` in CI (`.github/workflows/ci.yml:33`) |
