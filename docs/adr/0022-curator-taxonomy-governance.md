...
**AMENDED (append-only) — [ADR-0041](0041-stratum-kind-first-layout.md) (Proposed, KB wiki schema 2) RETARGETS §A's no-loss floor and SUPERSEDES one clause of the #57 addendum.** §A splits into three legs: the PATH leg retires (a note no longer needs a domain to have a path, so `normalize_plan`'s `domains[0]` path fallback goes); the CLASSIFICATION leg becomes `subjects: []` (asserting nothing, losing nothing — strictly more honest than asserting a possibly-false `domains[0]`); the RAW SHARD-KEY leg KEEPS `domains[0]`, because `raw/<domain>/<event_id>.md` still needs a directory and `raw/` never moves. The floor retires at BOTH sites §A names — `normalize_plan` AND the `vault_import.py` import lane §A says it "unifies with" (`vault_import.py:359`'s `first_domain` catch-all becomes `wiki/concepts/<slug>.md`). §B and §D are UNTOUCHED. **§C is AMENDED:** its `CREATE_DOMAIN` taxonomy write and `taxonomy_policy` gate stand (still unimplemented), but its worker duty "and **creates the lazy MOC**" LOSES ITS REFERENT — that MOC is `wiki/<domain>/<domain>-moc.md` and a domain no longer implies a map. Decision: a new domain materialises NOTHING structural; maps become editorial artefacts, because auto-seeding one per domain would recreate the empty-MOC-stub population #146 measured beating real answers. §C's atomic-diff guarantee is unaffected (taxonomy write + the run's concepts). **§E's tuning surfaces are KEPT as surfaces, but `curator.lint.max_orphans`'s SEMANTICS move with L2-1** (ADR-0010's banner): the orphan predicate stops being `type == "theme"` and becomes `kind in {concept, summary}`, with `entity` and `person` EXEMPT — both would otherwise be orphans by construction and would push a repo past a threshold that gates a run. **Superseded:** the #57 addendum §1 clause *"widening that regex was explicitly rejected"* — `core/pathsafe.py` is a closed Unicode-CATEGORY allowlist and therefore preserves the closed-set property that made the ASCII regex safe. That addendum's OTHER rejection (transliteration tables, because a versioned table renames future notes for identical input) STANDS, and the `note-<sha8>` fallback it chose STAYS as the last resort. **One further addendum consequence:** §2 ("Aliases: skip + count") rests on the premise that preserving a Korean alias "would require widening the closed alias/basename token grammar" — ADR-0041 D4.4 REMOVES that premise, since aliases run through the same slugger (`ollama_brain.py`: `alias = _slugify(raw_alias)`), so Korean aliases are now PRESERVED and `aliases_skipped_unslugifiable` becomes a residual counter rather than the common path. The prose below is retained verbatim for history.
