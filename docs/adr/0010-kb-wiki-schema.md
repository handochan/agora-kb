...
**SUPERSEDED IN PART (append-only) — [ADR-0041](0041-stratum-kind-first-layout.md) (Proposed) replaces this ADR's LAYOUT and NOTE-TYPE sections with KB wiki schema 2 (directory is the KIND; the subject leaves the path for `subjects:`).** SUPERSEDED: the *"Note types (exactly four)"* table, the *"Folder & naming rules"* block, the `type:` line of the frontmatter spec, and the `moc`/`daily` per-type frontmatter blocks. AMENDED (D-level): D3 (the `raw/` engine-written class gains `raw/_blob/`; `raw/<domain>/<inbox-id>.md` is unchanged), D5 (the child-bullet GRAMMAR is unchanged; the admitted CHILD SET changes), D6 (the taxonomy stays a fixed read-only INGEST input; L1-5's domain check moves from the path to `subjects:`). KEPT VERBATIM (D-level): D1, D2, D4, and the heading-slug anchor algorithm.
**The L1 ruleset, rule by rule (the enumeration below is exhaustive over L1-1…L1-20 — a partial one is how a rule silently keeps a retired predicate).** AMENDED: **L1-4** (required frontmatter keys on `kind`, not `type`; the common base gains a REQUIRED `kb:`; and the `body_status` value — which the v1 schema DOC already states as prose ("`body_status`, when present, must equal `pending`", `body_status ∈ {pending}`) but the v1 LINTER never checked — becomes an enforced L1-4 value check on `concept`/`summary`/`note`, closing that doc/code gap. Consequence for the ADR-0041 D6 importer: a v1 note carrying any other literal lints clean today and must be normalised or stripped by `agora import --from-kb`), **L1-5** (domain read moves from path to `subjects:`), **L1-6** (predicate `type in (moc, index)` → `kind in (map, index)`; the set-equality check itself unchanged), **L1-7 / L1-8 / L1-8b / L1-10 / L1-19** (all five are gated on `if n.type == "theme":` in `schema/lint.py`; the predicate becomes `kind in {concept, summary}`, with `entity` EXCLUDED from L1-7's non-stub-needs-sources rule), **L1-9** (`wiki/people/` carve-out), **L1-11** (`type` → `kind`, cross-checked against the directory), **L1-14** (daily date rule → note date/shard rule). KEPT VERBATIM: **L1-1 / L1-2 / L1-3** and **L1-15** (with the rider that `wiki/people/**` basenames are OUTSIDE the global basename identity space, so they neither collide with nor are resolvable by `[[basename]]`), **L1-12**, **L1-13**, **L1-16 / L1-17 / L1-18**, and **L1-20** — body-sentinel integrity, which exists in `schema/lint.py` but is MISSING from the L1 table below (the table stops at L1-19); this banner adds it to the ruleset of record. ADDED by ADR-0041 D3.1: **L1-22** (a `wiki/` segment-1 directory outside the closed kind set), **L1-23** (a `domains` entry beginning with `_` — the `raw/` reserved-prefix namespace), **L1-24** (a map `children:` bullet whose child's kind is not admitted — which finally gives the v1 "dailies MUST NOT appear in `children:`" rule an enforcement point; it is prose with no lint rule today). **L1-21 stays PRE-RESERVED** by `schema/lint.py` for the L2-6 promotion after `agora repo upgrade` (#63) and must not be reused.
**L2 is NOT kept verbatim:** **L2-1**'s orphan predicate is `n.type == "theme"` in three places (`schema/lint.py`, `faces/mcp_server.py` ×2) and `theme` does not exist in schema 2 — it becomes `kind in {concept, summary}`, with `entity` and `person` EXEMPT (both are orphans by construction: entities may not be map children, and the curator may not link into `people/` at all). This changes what `curator.lint.max_orphans` counts, and that key gates a run. L2-2…L2-6: KEPT. The prose below is retained verbatim for history.
