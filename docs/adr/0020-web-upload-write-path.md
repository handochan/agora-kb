...
**SHIPPED (append-only, 2026-09-04, #153) — this ADR's deferred half is no longer deferred, and the transport is not the shape the paragraph above predicted.** The original bytes ARE now stored verbatim: `Inbox.write(attachments=[(filename, media_type, data)])` stages them, content-addressed, at `_kb/inbox/<writer>/_attach/<sha256>.<ext>` inside the writer's own namespace (an `attachments:` frontmatter list on the event names them — **not** `raw_ref`, which is untouched and still means "a ref supplied by the caller"), and APPLY's `_materialize_attachments` writes `raw/_blob/<ab>/<sha256>.<ext>` + its `.meta.yaml` sidecar and cites the blob in the note's `sources:`. Decision 3 is intact: APPLY, the deterministic engine, is still the only writer of `raw/`. The two capture surfaces are `POST /api/upload` (+ `/api/upload-batch`, the HTMX form) and the new `agora capture --file`; `kb_remember` stays text-only. See the ADR-0041 addendum *"as-built: the capture transport and the `raw/_blob/` sidecar"*, and INGEST-CONTRACT §0.1 for the surface contract. **Two Consequences below are now historical:** *"the original binary is not stored verbatim"* and *"the drift-detection sidecar … is absent until binary staging lands"* — the `raw/_blob/` sidecar exists, though it carries CAPTURE facts, not the §2 re-ingest-drift key set. One property the wave did NOT change: a candidate the curator **DROPs** writes no note, so no `sources:` cites its bytes and APPLY materialises nothing — the artefact stays in the git-ignored `_kb/processed/<date>/_attach/` spool, exactly as a DROPped free-text capture writes no `raw/<domain>/<event_id>.md`.
